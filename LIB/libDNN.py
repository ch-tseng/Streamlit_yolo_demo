...
class opencvYOLO:
    def __init__(self, imgsize=(416,416), objnames="coco.names", \
            weights="yolov3.weights", darknetcfg="yolov3.cfg", \
            tcolors=None, score=0.25, nms=0.6, gpu=False):
        ...
        if tcolors is not None and len(tcolors)>0:
            self.tcolors = tcolors

        else:
            self.tcolors = []
            for i in range(0, len(self.classes )):
                self.tcolors.append( (random.randint(0,255),random.randint(0,255),random.randint(0,255))  )
    ...
    def getObject(self, frame, score, nms, labelWant='', drawBox=False, char_type='Chinese'):
        ...
        if self.mtype == 'yolov5':
            ...
            self.objCounts = len(self.classIds)

            if(drawBox==True):
                for id, class_id in enumerate(self.classIds):
                    xmin, ymin, xmax, ymax = self.bbox[id][0], self.bbox[id][1], \
                                             self.bbox[id][0]+self.bbox[id][2], self.bbox[id][1]+self.bbox[id][3]
                    frame = self.drawPred(frame, class_id, self.scores[id], bold, tcolors[class_id],
                            xmin, ymin, xmax, ymax, type=char_type)

        #frame = self.draw_det(self.bbox, self.labelNames, frame)
        self.frame = frame

        return frame

    def draw_det(self, bboxes, subject, img):
        ly, plate_h = None, None
        for i, (x,y,w,h) in enumerate(bboxes):
            if subject[i] == 'plate':
                plate_y, plate_x, plate_h, plate_w = y, x, h, w
                ly = y+h
        ...
        for i, (x,y,w,h) in enumerate(bboxes):
            print('w/25', w/25)
            if subject[i] != 'plate':
                font, fontsize, fontbold = cv2.FONT_HERSHEY_TRIPLEX, w/15, 1
                if fontsize<0.9:
                    fontsize = 0.9
                    fontbold = 1
                if fontsize>1.5:
                ...
                color = self.tcolors[i]
                cv2.rectangle(img, (x, y), (x+w, y+h), color, fontbold)
                print(fontsize, fontbold)
                cv2.putText(img, subject[i], (x,y-5), cv2.FONT_HERSHEY_PLAIN, fontsize, color, fontbold)
